...
**推論だけでなく学習もできる。** ONNX のグラフをそのまま逆伝播させるので、モデルを別の形で
書き直す必要がない。
...
```
before: "うま味のある" (conf 1.000)
target: 6 ids, first changed 11069 -> 11070
  step   0  loss  16.1193   |g|    3.304
  step  24  loss   0.0000   |g|    0.001
after:  "えま味のある" (conf 1.000)
loss 16.1193 -> 0.0000  (100.0% down)
```

1 サンプルへの過学習で、**狙った 1 文字だけが実際に書き換わる**。前向き・逆伝播 30 種・勾配累積・
Adam・CTC 損失のどれか 1 つでもずれていたら、この結果にはならない。

### 自分のデータで追加学習する

ラベルは PaddleOCR と同じ `<画像パス>	<テキスト>` の 1 行 1 サンプル。パスはリストファイルからの
相対で、データセットのフォルダごと移動できる。

```sh
# 手元にデータがなければ、まず動くことを確かめるための合成データを作る
python tools/make_rec_data.py --out scratch/recdata --n 300 --val 80

./ppocr.exe train --data scratch/recdata/train.txt --val scratch/recdata/val.txt                   --steps 60 --batch 4 --lr 2e-4 --eval-every 20 --out ft.bin
```

```
rec: 746 nodes, dict 18385 classes
data: 300 train, 80 val
before: val exact 32.5% (26/80)
training 68 tensors (4.10 M values) on 300 samples
  step    20  loss   0.2133   val exact 100.0% (80/80)
  step    40  loss   0.0002   val exact 100.0% (80/80)
  step    60  loss   0.0000   val exact 100.0% (80/80)
```

`val` は `train` と**文字列が重複しない**ように作ってあるので、この 100% は暗記ではない。ただし
**合成データは 1 フォント・白背景・語彙 25 語**なので、この数字が言えるのは「前向き・逆伝播・
Adam・CTC が実モデル上で噛み合っている」ことまでで、汎化の証拠ではない。汎化を見たければ自分の
データを食わせる。

書き出した `ft.bin` は元の `.onnx` を書き換えず、`名前 -> float 配列`の**差分**として持つ。16 MB の
protobuf を再エンコードせずに済み、fine-tune を後から捨てられる。

```sh
./ppocr.exe run --img photo.jpg --weights ft.bin     # 推論に効かせる
./ppocr.exe train --resume ft.bin --data ... --steps 200   # 続きから学習する
```

| オプション | 既定 | 意味 |
| --- | --- | --- |
| `--data` / `--val` | 必須 / 省略可 | 学習・検証のリストファイル |
| `--steps` | 500 | 最適化の更新回数（サンプル数ではない） |
| `--batch` | 4 | 1 更新あたりのサンプル数 |
| `--lr` | 1e-4 | Adam の学習率 |
| `--clip` | 5.0 | 勾配のグローバルノルムの上限 |
| `--eval-every` | 100 | 何 step ごとに検証するか |
| `--img-h` | 48 | 入力の高さ。モデルに合わせる |
| `--resume` | — | 既存の重みファイルから続ける |

**辞書にない文字はエラーで、黙って飛ばさない。** ラベルから 1 文字抜けた状態で学習すると、
「その文字を出さない」ことを学習してしまう。損失は下がるので、気づけない。
...
1. `infer_only()` を立てない（各 op が grad バッファを持ち、`gr::tape` がクロージャを記録）
2. 出力を `nograd()` に通さない（そのクロージャを剥がさない）
3. 生存解析による解放をしない（前向きの最後の読み手が済んでも逆伝播がまだ読む）

学習用にインタプリタをもう 1 本書けば 400 行の ONNX デコードが二重になり、どちらかを直した瞬間に
ズレる。`gr::` のラッパーは既存の前向き関数を呼んでからテープを張るだけで、`infer_only()` の
ときは素通りするので、増えるのは**ノードあたり分岐 1 つ**（要素あたりではない）。

実測（同じ画像・同じコマンドで HEAD 版と比較、時間表示を除く）:

| | |
|---|---|
| `run japan_2.jpg` | 53 行 完全一致 |
| `run page_ja.png` | 26 行 完全一致 |
| `det page_ja.png` | 25 行 完全一致 |

### 検証

```sh
g++ -std=c++17 -O1 -o ctc_check.exe  tools/ctc_check.cpp  && ./ctc_check.exe
g++ -std=c++17 -O2 -o grad_check.exe tools/grad_check.cpp && ./grad_check.exe
```

* `grad_check` — 逆伝播 30 種を中心差分と突き合わせる。**縮約に `sum()` を使わない**のが要点で、
  それだと全出力に同じ勾配 1 が流れるので `transpose` の置換ミスのような**並びの誤り**が
  素通りする。出力ごとに違う重みの内積にすると並びが観測可能になる。
* `ctc_check` — 数値微分に加えて **各時刻の勾配の行和が −1** であることを検査する。パスは各時刻
  ちょうど 1 つの記号を占有するので、これは必ず成り立つ。1 点の数値微分では見逃す添字のバグが
  まとめて捕まる。

### 踏んだ罠

* **`Pow` の指数 `2.0` も ONNX 上はただの float 定数**で、学習対象に入れると Adam が 1 ステップで
  `2.0 → 2.0003` にする。`pow(負数, 非整数)` は NaN なので、LayerNorm の**平均以下の要素だけが
  NaN になり平均以上は正常**という、算術的に説明のつかない壊れ方をする。パラメータかどうかは
  **グラフ上の位置**（畳み込みのカーネル・バイアス、行列積の右辺、正規化のスケールとシフト）で
  決める。247 個 → 68 個に減ったが、値の総数は 4.11M → 4.10M でほぼ変わらない。
* **「勾配を受け取れる」と「最適化される」は別**。非パラメータもテープ上では親なので grad バッファは
  要る。一緒にすると、パラメータを絞った瞬間に範囲外書き込みで落ちる。
* **逆伝播を持たない op は実行時エラーにならない**。テープに何も残らないまま `backward()` が
  素通りし、その先の勾配が全部ゼロになる。それでも**損失は下がる**（テープが届く層だけは学習する）。
  学習経路では微分可能な op を名前で列挙し、それ以外で停止する。
* **`nograd()` を呼ぶ前向き関数が 8 つある**（推論では grad の確保とゼロ埋めが演算より重いため）。
  テープを張る側で戻す必要があり、各ラッパーに書くと必ず忘れるので `gr::tape` の 1 箇所にまとめた。
* **`ew::reshape` だけ `make_tensor` を通らない**。同じ理由で grad が無い。

### BatchNorm の統計は凍結する

推論形（running mean/var によるアフィン変換）のまま、gamma / beta / x にだけ勾配を流す。手抜きでは
なく、ここでは正解:

1. **グラフにそれしか無い** — 書き出された ONNX は running 統計を持ち、再計算する機構は持たない
2. **追加学習のバッチは 1〜8 行**。この程度の標本から推定したバッチ統計は事前学習済みバックボーンを
   壊すほどノイジーで、fine-tuning で BN を凍結するのは標準的な作法
3. **前向きが「同じバッチに何が入っているか」に依存しなくなる**ので、1 サンプル単位で再現・検証できる
